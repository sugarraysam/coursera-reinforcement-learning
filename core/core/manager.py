...
class Manager:
    ...
    def compute_values_table(self, values):
        ...
        for state in range(len(values)):
            self.values_table[
                np.unravel_index(state, (self.grid_h, self.grid_w))
            ] = values[state]
    ...
    def visualize(self, values, episode_num):
        if not hasattr(self, "fig"):
            ...
            if self.true_values_file is not None:
                ...
                self.cmap_VE.set_bad("black", 1.0)
        ...
        plt.yticks([])
        im = plt.imshow(
            self.values_table, cmap=self.cmap, interpolation="nearest", origin="upper"
        )
        ...
        if self.true_values is not None:
            ...
            plt.yticks([])
            im = plt.imshow(
                (self.values_table - self.true_values) ** 2,
                origin="upper",
                cmap=self.cmap_VE,
            )
    ...
    def run_tests(self, values, RMSVE_threshold):
        assert self.true_values is not None, (
            "This function can only be called once the true values are given during "
            + "runtime."
        )
        self.compute_values_table(values)
